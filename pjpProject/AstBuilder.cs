...
{
    public List<Stmt> BuildProgram(LanguageParser.ProgramContext ctx)
    ...
    {
        var elemType = GetVarType(ctx.type());
        var size     = int.Parse(ctx.INT_LIT().GetText());
        var name     = ctx.ID().GetText();
        return new ArrayDeclStmt(elemType, size, name, ctx.Start.Line);
    }

    public override object? VisitFileDeclStmt(LanguageParser.FileDeclStmtContext ctx)
    {
        var names = ctx.ID().Select(id => id.GetText()).ToList();
        return new FileDeclStmt(names, ctx.Start.Line);
    }

    public override object? VisitFopenStmt(LanguageParser.FopenStmtContext ctx)
    {
        var varName  = ctx.ID().GetText();
        var raw      = ctx.STR_LIT().GetText();
        var fileName = raw[1..^1];
        return new FopenStmt(varName, fileName, ctx.Start.Line);
    }

    public override object? VisitFileWriteStmt(LanguageParser.FileWriteStmtContext ctx)
    {
        var varName = ctx.ID().GetText();
        var values  = ctx.expr().Select(e => (Expr)Visit(e)!).ToList();
        return new FileWriteStmt(varName, values, ctx.Start.Line);
    }
    ...
    public override object? VisitIndexExpr(LanguageParser.IndexExprContext ctx)
    {
        var target = (Expr)Visit(ctx.expr(0))!;
        var index  = (Expr)Visit(ctx.expr(1))!;
        return new IndexExpr(target, index, ctx.Start.Line);
    }
